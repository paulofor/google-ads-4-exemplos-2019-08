...
import com.google.api.ads.adwords.axis.v201809.cm.LocationCriterionServiceInterface;
...
public class LookupLocation {
	...
	public static void runExample(AdWordsServicesInterface adWordsServices, AdWordsSession session,
			String[] locationNames) throws RemoteException {
		// Get the LocationCriterionService.
		LocationCriterionServiceInterface locationCriterionService = adWordsServices.get(session,
		...
		for (LocationCriterion locationCriterion : locationCriteria) {
			...
			System.out.printf(
					"The search term '%s' returned the location '%s (%d)' of type '%s' "
							+ "with parent locations '%s' and reach %d (%s).%n",
					...
					parentString, locationCriterion.getReach(), locationCriterion.getLocation().getTargetingStatus());
		}
	}
}
